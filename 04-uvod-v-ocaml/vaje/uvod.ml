...
let rec sum_int_pairs par =
  match par with
  | [] -> []
  | (x, y) :: ostalo -> [x + y] :: sum_int_pairs ostalo
...
let rec get k sez =
  match sez with
    | [] -> failwith "Joj ne"  
    | x :: ostalo -> if k <= 0 then x else  get (k - 1) ostalo 
...
let rec insert x k list =
  match list with
    | [] -> [x]
    | y :: ostalo -> if k <= 0 then x :: y :: ostalo else y :: insert x (k - 1) ostalo
...
(*let rec max_on_components' list1 list2 =
  let pomozni list1 list2 list =
    match (list1, list2) with
      | _, [] | [], _ -> list 
      | (x :: xs, y :: ys) -> if x > y then pomozni xs ys (x :: list) else pomozni xs ys (y :: list)
  in
  pomozni list1 list2 []*)
