...
(*List.rev naredi tocno to kot reverse *)
...
let rec unzip_tlrec list =
  let rec unzip' list list1 list2 =
    match list with
    | [] -> (reverse list1, reverse list2)
    | (x, y) :: xs -> unzip' xs (x :: list1) (y :: list2)
  in 
  unzip' list [] []
...
let rec loop condition f x =
    if condition x then let x' = f x in loop condition f x' 
    else x
